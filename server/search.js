...
router.post('/', (req, res) => {
  ...
  const cityState = cities
    .filter(cit => cit.name.match(req.body.city))
    .sort((a, b) => b.population - a.population)[0].adminCode;

  // gets a generic zipCode if none is given
  const zipCode = zipcodes.lookupByName(req.body.city, cityState);
